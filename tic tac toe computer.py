...
def computer_move():
    global turn
    if win() is not True and turn < 8:
        computer = randint(0, 8)
        if squares[computer]["text"] == "":
            squares[computer].config(text="O")
            turn += 1
        else:
            computer_move()
        win()
...
def win():
    ...
    for option in win_possible:
        if len(set(option)) == 1:
            sign = option[0]
            if sign == "X":
                label.config(text=f"YOU WIN!", fg="#e7305b")
            else:
                label.config(text=f"YOU LOSE!", fg="#e7305b")
            return True
